#!/usr/bin/env python3
"""Memory-efficient three-GPU DPO initialized from frozen B10."""
...
def single_row(rows: list[dict[str, Any]]) -> dict[str, Any]:
    if len(rows) != 1:
        raise ValueError("v0.37A DPO requires per-device batch size 1")
    return rows[0]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--experiment-name", default="v037a-b10-dpo")
...
def main() -> None:
    ...
    if any(any(key not in row for key in required) for row in rows):
        raise ValueError("Reference JSONL is missing frozen B10 log probabilities")
    ...
    if args.resume_from:
        state_path = Path(args.resume_from) / "v037a_training_state.pt"
    ...
    metadata = {
        "objective": "standard DPO with precomputed frozen-B10 reference log-probabilities",
            ...
            try:
                ...
                with accelerator.accumulate(model):
                    with torch.no_grad():
                        ...
                        chosen_coefficient = -args.beta * sigmoid_negative
