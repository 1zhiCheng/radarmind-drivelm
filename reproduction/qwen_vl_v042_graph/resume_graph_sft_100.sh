...
RESUME=$OUT/checkpoint-100
RUN=/mnt/data/zzy/drivelm/reproduction/qwen_vl_v042_graph

GPU0=GPU-9e49353e-f137-3d6d-f7d5-987593a56d30
GPU2=GPU-07f03596-83e4-920f-9669-fbec78272f67
GPU3=GPU-8f2fb421-991d-f5eb-c811-448f7e105297

test -s "$RESUME/adapter_model.safetensors"
test -s "$RESUME/v042_training_state.pt"

CUDA_VISIBLE_DEVICES="$GPU0,$GPU2,$GPU3" "$PY" -m accelerate.commands.launch \
  --multi_gpu \
  --num_processes 3 \
  ...
  --max-length 8192 \
  --seed 42 \
  2>&1 | tee -a "$RUN/logs/graph_sft.log"
