#!/usr/bin/env python3
"""Continue a frozen-start DriveLM LoRA with pure assistant-token CE."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--experiment-name", default="v039a-mol-ce")
    parser.add_argument("--model-path", required=True)
    parser.add_argument("--adapter-path", required=True)
    parser.add_argument(
        "--resume-from-checkpoint",
        default=None,
        help="Resume adapter and AdamW state; --max-steps remains the total target step.",
    )
...
def main() -> None:
    ...
    if args.resume_from_checkpoint:
        state_path = Path(args.resume_from_checkpoint) / "v036_training_state.pt"
        if not state_path.is_file():
            raise FileNotFoundError(f"Missing resumable training state: {state_path}")
    ...
    if total_steps <= start_step or total_steps > available_updates:
        raise ValueError(
            f"max-steps {total_steps} is invalid; start={start_step}, "
            f"available updates={available_updates}"
        )

    metadata = {
        "policy_initialization": str(Path(args.adapter_path).resolve()),
        "resumed_from": (
            str(Path(args.resume_from_checkpoint).resolve())
    ...
    for _epoch in range(args.epochs):
        for batch_index, batch in enumerate(loader):
            # Checkpoints are written only at optimizer boundaries. Replaying the
            # deterministic shuffle and skipping consumed micro-batches preserves
            # the next sample without storing a large sampler state.
    ...
    if accelerator.is_main_process:
        report = {
            ...
            "mean_loss_since_resume": cumulative_loss / max(global_step - start_step, 1),
