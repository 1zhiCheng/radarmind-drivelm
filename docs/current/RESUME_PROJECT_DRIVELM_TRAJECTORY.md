...
- 面向 Agentic RL 搭建 trajectory 训练链路：冻结 Perception/Prediction 专家生成无 gold 泄漏的轨迹状态，仅优化 Planning policy；实现 API-free 组合奖励、四路 rollout、KL 约束以及三卡 GRPO/GSPO 等预算对照流水线，当前处于训练与完整 dev 验证阶段。
...
- 搭建无标签泄漏的 trajectory RL 流水线，以上游专家预测构造决策状态，使用可解释组合 reward、vLLM rollout、KL 约束及 verl/FSDP 在 3×RTX 5090 上进行 GRPO/GSPO 公平对照；构建可缓存、可审计的官方结构代理评测与自动晋级机制。

## 面试时的 60 秒介绍

这个项目不是普通的图像问答微调，而是把 DriveLM 的六路环视图像组织成“感知、预测、规划、行为”四级自动驾驶推理 Agent。我先用严格控制变量的 CE-LoRA 得到 Qwen2.5-VL-7B 基线，再完整验证 DPO；实验发现标准 DPO 虽提高多选题，却会损害开放式 planning，因此没有只报有利指标，而是增加 CE anchor、全量 dev 和晋级门控。随后，我将四类任务拆成独立 LoRA 专家并按问题任务路由，最终把本地 DriveLM-DS 从 0.59464 提升到 0.60824。当前进一步把单题训练改造成真实的 trajectory context：上游感知和预测由冻结模型生成，规划策略看不到 gold 上下文，再在相同数据、reward 和 rollout 预算下比较 GRPO 与 GSPO。
...
冻结上游专家生成 trajectory state（无 gold 泄漏）
          |
          v
Planning policy：GRPO vs GSPO -> 3,355 QA 完整 dev 评测
```
...
- trajectory 数据构建、奖励函数、三卡训练和自动验收流水线已经落地；在 GRPO/GSPO 全量结果完成前，简历中只写“训练与验证阶段”，不写 RL 带来的指标提升。
